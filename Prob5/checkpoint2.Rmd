...
``` {r,  warning=FALSE}
ggplot(womenLike, aes(womenLike$attr, womenLike$like)) +
  geom_point(alpha = 0.1, position = position_jitter(width = 0.3), color="purple4") +
  labs(title="Avaliações por pessoas do sexo feminino", x= "Nota em atração", y="Nota em Like")


ggplot(menLike, aes(menLike$attr, menLike$like)) +
  geom_point(alpha = 0.1, position = position_jitter(width = 0.3), color="springgreen4") +
  labs(title="Avaliações por pessoas do sexo masculino", x= "Nota em atração", y="Nota em Like")
...
ggplot(womenLike, aes(womenLike$attr, womenLike$like)) +  geom_point(alpha = 0.1, position = position_jitter(width = 0.3), color="purple4") + 
  labs(title="Previsão do modelo para grupo do sexo feminino", x= "Nota em atração", y="Nota em Like") +  
  geom_line(aes(y = predict(womenLikeMod, womenLike)), colour = "red")

ggplot(menLike, aes(menLike$attr, menLike$like)) +  
  geom_point(alpha = 0.1, position = position_jitter(width = 0.3), color="springgreen4") +
  labs(title="Previsão do modelo para grupo do sexo masculino", x= "Nota em atração", y="Nota em Like") + 
  geom_line(aes(y = predict(menLikeMod, menLike)), colour = "red")

``` 

Tendo computado os valores previsto, é possível agora visualizar os resíduos do modelo. 


``` {r,  warning=FALSE}
ggplot(womenLikeMod, aes(attr, .resid)) + 
  labs(title="Resíduos do modelo para grupo do sexo feminino", x= "Nota em atração", y="Resíduos") +
  geom_point(alpha = 0.1, color="purple4") +
...
```
## Notas em Likes x Notas em Interesses e Hobbies em comum



```{r}
uniqueId =  as.data.frame(unique(data$id))
```